...
class CodeGenerator:
    ...
    def _dump_debug_mlir(self, file_name: str, ir_obj: Any):
        """Write a debug MLIR artifact when DEBUG=1 is enabled."""
        if os.getenv("DEBUG") != "1" or self.project_dir is None:
            return
        try:
            debug_dir = os.path.join(self.project_dir, "build", "debug")
            os.makedirs(debug_dir, exist_ok=True)
            with open(
                os.path.join(debug_dir, file_name), "w", encoding="utf-8"
            ) as f:
                f.write(ir_obj if isinstance(ir_obj, str) else str(ir_obj))
        except Exception:
            # Debug dumping should never affect the actual lowering flow.
            pass
    ...
    def build_core_function(
        ...
        """
        func_name = original_func.attributes["sym_name"].value
        ...
        self._dump_debug_mlir(f"{func_name}_preprocessed.mlir", func_string)
        original_module = aie_ir.Module.parse(func_string)
        self._dump_debug_mlir(f"{func_name}_parsed.mlir", original_module)
        ...
        with aie_ir.InsertionPoint(entry_block):
            ...
            self._dump_debug_mlir(f"{func_name}_core_skeleton.mlir", self.aie_module)
    ...
    def aie_codegen(
        ...
        with aie_ir.Context() as ctx, aie_ir.Location.unknown():
            ...
            self._dump_debug_mlir("aie_wrapper.mlir", self.aie_module)
            ...
            with aie_ir.InsertionPoint(end_op):
                ...
                for func in core_funcs:
                    ...
                    self._dump_debug_mlir(
                        f"aie_after_{func_name}.mlir",
                        self.aie_module,
                    )
