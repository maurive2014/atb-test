...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 4096 * sizeof(short),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(3));
  short *bufIn0 = bo_in0.map<short *>();
  std::vector<short> srcVec0;
  ...
  if (ifile0_size != 4096 * sizeof(short)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<short> vec0(4096);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 4096 * sizeof(short));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(short)));

  auto bo_out1 = xrt::bo(device, 4096 * sizeof(short),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 4096 * sizeof(short),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(5));
  short *bufIn2 = bo_in2.map<short *>();
  std::vector<short> srcVec2;
  ...
  if (ifile1_size != 4096 * sizeof(short)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<short> vec1(4096);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 4096 * sizeof(short));
  srcVec2.insert(srcVec2.end(), vec1.begin(), vec1.end());
  memcpy(bufIn2, srcVec2.data(), (srcVec2.size() * sizeof(short)));
  ...
  short *bufOut1 = bo_out1.map<short *>();
  std::ofstream ofile2("output2.data", std::ios::binary);
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 4096 * sizeof(short));
  ofile2.close();
  ifile1.close();
  if (trace_size > 0) {
